...
namespace Cofradinn.Utilities
{
    public static class Math3D
    {
        ...
        /// Km
        /// </summary>
        public const float _EARTH_RADIUS = 6378.0f;
        ...
        /// https://www.ecured.cu/Distancia_de_un_punto_a_una_recta#Ejemplos
        /// </summary>
        public static double __DistanceBetweenPointAndLine(Vector2 LP1, Vector2 LP2, Vector2 point)
        {
            // Calculating slope
            double m = __GetLineSlope(LP1, LP2);

            // Equation of the line: https://oa.ugto.mx/oa/oa-enmsir-0000001/2_formas_de_la_ecuacin_de_la_recta.html
            // Y = m*X + B     (Ordinary: Pendiente - ordenada)
            // Ax + By + C = 0 (General - implicita)
            // (x/a) + (y/b) = 1 (Simetryc: "a" and "b" son los cortes con los ejes de coordenadas)

            // Calculating equation of the line, model point-slope 
            // Ax + By + C = 0 (General - implicita)
            // y-y1 = m(x - x1)
            // y-y1 = m*x - m*x1
            // y-y1 = m*x - m*x1
            // y - m*x + m*x1 -y1 = 0
            // (y1-y2)x + (x2-x1)y + (x1*y2 - x2*y1) = 0 // https://en.wikipedia.org/wiki/Linear_equation
            //     (A)x +     (B)y +      (C)        = 0 (General - implicita)


            double A = (LP1.y - LP2.y); // (y1-y2)
            double B = (LP2.x - LP1.x); // (x2-x1)
            double C = ((LP1.x * LP2.y) - (LP2.x * LP1.y)); //(x1*y2 - x2*y1)

            // distance between point and line: https://www.ecured.cu/Distancia_de_un_punto_a_una_recta#Ejemplos
            //           |A*Px + B*Py + C|  
            // d(P,r) = ________________
            //           \|(A*A + B*B)   

            // double distance_P_R = Math.Abs((A * point.x) + (B * point.y) + C) / Math.Sqrt(A * A + B * B);
            // Quitamos valor absoluto para que me diga negativo cuando está a la izquierda y positivo a la derecha de la recta
            double distance_P_R = ((A * point.x) + (B * point.y) + C) / Math.Sqrt(A * A + B * B);
            return distance_P_R;
        }
        /// <summary>
        /// Calcular la pendiente de una recta
        /// </summary>
        public static double __GetLineSlope(Vector2 p1, Vector2 p2)
        {
        ...
        /// <summary>
        /// Calcular el angulo de la recta, return degrees or radians
        /// <para>(https://es.slideshare.net/ivanarroyoaztk/pendiente-y-ngulo-de-inclinacin-de-una-recta-matematicas)</para>
        /// <para>(https://www.rapidtables.com/convert/number/how-radians-to-degrees.html)</para>
        /// </summary>
        public static double __GetLineAngle(Vector2 p1, Vector2 p2, bool degrees)
        {
            // Calculating slope
            double m = __GetLineSlope(p1, p2);
            double radiansangle = Math.Atan(m);
            if (degrees)
                return (radiansangle > 0 ? radiansangle : (2 * Math.PI + radiansangle)) * 360 / (2 * Math.PI);
            else
                return radiansangle;
        }
